...
#define USE_CTRL (1)
#define DATA_W (IOB_CACHE_AXI_CSRS_FE_DATA_W)
#define FE_NBYTES_W (2)
#define CACHE_DATA_ADDR_W (IOB_CACHE_AXI_CSRS_FE_ADDR_W)
// address control after data addressing
#define CACHE_CTRL_BASE (1 << (CACHE_DATA_ADDR_W))

static inline void use_ctrl() {
  iob_cache_axi_csrs_init_baseaddr(CACHE_CTRL_BASE);
}

static inline void use_data() { iob_cache_axi_csrs_init_baseaddr(0); }
...
int iob_core_tb() {
  ...
  failed += lru_test(IOB_CACHE_AXI_CSRS_NWAYS_W, IOB_CACHE_AXI_CSRS_NLINES_W);
