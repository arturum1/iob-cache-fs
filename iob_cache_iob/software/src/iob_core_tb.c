...
#define USE_CTRL (1)
#define DATA_W (IOB_CACHE_IOB_CSRS_FE_DATA_W)
#define FE_NBYTES_W (2)
#define CACHE_DATA_ADDR_W (IOB_CACHE_IOB_CSRS_FE_ADDR_W)
// address control after data addressing
#define CACHE_CTRL_BASE (1 << (CACHE_DATA_ADDR_W))

static inline void use_ctrl() {
  iob_cache_iob_csrs_init_baseaddr(CACHE_CTRL_BASE);
}

static inline void use_data() { iob_cache_iob_csrs_init_baseaddr(0); }
...
void print_counters() {
  ...
  printf("\tRW Hit:%d\n", iob_cache_iob_csrs_get_RW_HIT());
  printf("\tRW Miss:%d\n", iob_cache_iob_csrs_get_RW_MISS());
  printf("\tRead Hit:%d\n", iob_cache_iob_csrs_get_READ_HIT());
  printf("\tRead Miss:%d\n", iob_cache_iob_csrs_get_READ_MISS());
  printf("\tWrite Hit:%d\n", iob_cache_iob_csrs_get_WRITE_HIT());
  printf("\tWrite Miss:%d\n", iob_cache_iob_csrs_get_WRITE_MISS());
}

void wtb_status() {
  use_ctrl();
  printf("\tWrite Buffer Status:\n");
  printf("\t\tEmpty: %d\n", iob_cache_iob_csrs_get_WTB_EMPTY());
  printf("\t\tFull: %d\n", iob_cache_iob_csrs_get_WTB_FULL());
}

void reset_counters() {
  use_ctrl();
  iob_cache_iob_csrs_set_RST_CNTRS(1);
}
...
int ctrl_test() {
  ...
  printf("CTRL Test\n");
  use_ctrl();
  printf("\tVersion: %x\n", iob_cache_iob_csrs_get_version());
  ...
  iob_cache_iob_csrs_set_INVALIDATE(1);
  iob_cache_iob_csrs_set_INVALIDATE(0);
...
int iob_core_tb() {
  ...
  failed += lru_test(IOB_CACHE_IOB_CSRS_NWAYS_W, IOB_CACHE_IOB_CSRS_NLINES_W);
